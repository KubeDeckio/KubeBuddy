...
func collectPrometheusMetrics(nodes []map[string]any, opts ClusterDataOptions) (*ClusterMetrics, error) {
	...
	queries := map[string]string{
		"NodeCpuUsagePercent":    `(1 - avg by(instance)(rate({__name__=~"node_cpu_seconds_total|kubernetes.io/anthos/node_cpu_seconds_total",mode="idle"}[5m]))) * 100`,
		"NodeMemoryUsagePercent": `(1 - ({__name__=~"node_memory_MemAvailable_bytes|kubernetes.io/anthos/node_memory_MemAvailable_bytes"} / {__name__=~"node_memory_MemTotal_bytes|kubernetes.io/anthos/node_memory_MemTotal_bytes"})) * 100`,
		"NodeDiskUsagePercent":   `100 * (1 - (sum by(instance) ({__name__=~"node_filesystem_avail_bytes|kubernetes.io/anthos/node_filesystem_avail_bytes",fstype!~"tmpfs|aufs|squashfs", device!~"^$"}) / sum by(instance) ({__name__=~"node_filesystem_size_bytes|kubernetes.io/anthos/node_filesystem_size_bytes",fstype!~"tmpfs|aufs|squashfs", device!~"^$"})))`,
	}
